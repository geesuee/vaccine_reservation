		...
		try {
			...
			while (esc == null) {
				...
				// 접종 예약자
					...
					Users user = UsersDAO.getUser(userName, idNum);  
					...
					if(user != null) {  // 접종 예약 정보 존재
						...
						while(true) {
							if(reservation == 100) {
								...
								// switch문 끝나고 나서 반복 잘 돌아가는지 확인 필요
								switch(function) {
								case 1: 
									System.out.println("\n========== 1) 백신 확인 ==========");
						...
						if(!hospitalSet.contains(sHospital)) {
							System.out.println("\n해당 병원은 선택이 불가합니다.");
							inputNum = 100;
							continue loop1;
						}
						...
						if(!vaccineList.contains(sVaccine) || HospitalDAO.getHospitalVaccine(sHospital, sVaccine) <= 0) {
							System.out.println("\n해당 백신은 예약이 불가합니다.");
							inputNum = 100;
							continue loop1;
						}
						...
						try {
							addUser = new Users(idNum, userName, age, address, sDate1, sDate2, selectVaccine, selectHospital);
						}catch(Exception e) {
							e.printStackTrace();
						}
